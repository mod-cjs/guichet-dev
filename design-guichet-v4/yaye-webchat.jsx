...
const YayeWebChatPage = () => {
  const yav = (size) => (
    <span style={{ width: size, height: size, borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg,#19A757,#0A807F)", color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontFamily: "Georgia,serif", fontSize: size * 0.5, boxShadow: "0 0 0 2px rgba(255,255,255,.25)" }}>Y</span>
  );
    ...
    return (
      <div style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 12, overflow: "hidden", marginTop: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 11px", background: t.soft }}>
          ...
          <span style={{ fontSize: 10.5, fontWeight: 900, color: t.c, textTransform: "uppercase", letterSpacing: ".4px" }}>{t.label}</span>
          <span style={{ flex: 1 }} /><span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 10.5, fontWeight: 800, color: t.c }}><svg className="gj-icon" style={{ width: 11, height: 11 }}><use href="#i-clock" /></svg>{duree}</span>
        </div>
        <div style={{ padding: "10px 11px", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <button style={{ background: t.c, color: "#fff", border: 0, minHeight: 38, borderRadius: 8, fontWeight: 800, fontSize: 12, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6 }}>{action}<svg className="gj-icon" style={{ width: 12, height: 12 }}><use href="#i-arrow-right" /></svg></button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={{ display: "flex", height: "100%", background: "var(--gj-bg)", overflow: "hidden", fontFamily: "var(--gj-font-sans)" }}>
      ...
      <div style={{ width: 400, flexShrink: 0, background: "#fff", borderLeft: "1px solid var(--gj-line)", boxShadow: "-12px 0 40px rgba(0,0,0,.12)", display: "flex", flexDirection: "column", position: "relative", zIndex: 2 }}>
        ...
        {/* messages */}
        <div style={{ flex: 1, overflowY: "auto", background: "#F5FAF8", padding: "14px 16px", display: "flex", flexDirection: "column", gap: 9 }}>
