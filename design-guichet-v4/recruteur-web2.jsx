            ...
            <div style={{ display: "flex", gap: 8 }}>
              {days.map(([d, n], i) => (
                <button key={n} style={{ flex: 1, padding: "9px 0", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", textAlign: "center", background: i === 2 ? "var(--gj-blue)" : "#fff", border: i === 2 ? 0 : "1.5px solid var(--gj-line)", color: i === 2 ? "#fff" : "var(--gj-ink)" }}>
                  <div style={{ fontSize: 10.5, fontWeight: 700, opacity: .8 }}>{d}</div>
            ...
            return (
              <div key={th.id} onClick={() => setActive(th.id)} style={{ display: "flex", gap: 11, padding: "13px 18px", borderBottom: "1px solid var(--gj-line)", cursor: "pointer", background: on ? "var(--gj-blue-soft)" : "transparent" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><span style={{ fontSize: 13.5, fontWeight: 800, color: "var(--gj-ink)" }}>{th.who}</span><span style={{ fontSize: 10.5, color: "var(--gj-grey-2)", flexShrink: 0 }}>{th.time}</span></div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {t.msgs.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: m.me ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "68%", background: m.me ? "var(--gj-blue)" : "#fff", color: m.me ? "#fff" : "var(--gj-ink)", border: m.me ? 0 : "1px solid var(--gj-line)", borderRadius: m.me ? "14px 14px 4px 14px" : "14px 14px 14px 4px", padding: "10px 14px", fontSize: 13.5, lineHeight: 1.45 }}>{m.t}<div style={{ fontSize: 10, opacity: .6, marginTop: 4, textAlign: "right" }}>{m.time}</div></div>
            </div>
          ))}
        </div>
          ...
          return (
            <div key={it.id} style={{ ...rCard, padding: 16, display: "flex", alignItems: "center", gap: 15 }}>
              <div style={{ width: 64, flexShrink: 0, textAlign: "center", background: "var(--gj-blue-soft)", color: "var(--gj-blue-ink)", borderRadius: 11, padding: "9px 4px" }}>
                ...
                <div style={{ fontSize: 9.5, fontWeight: 800, marginTop: 3 }}>{it.day.split(" ").slice(0, 2).join(" ")}</div>
              </div>
              {rAvatar(it.init, 42)}
              <div style={{ flex: 1, minWidth: 0 }}>
