...
const YayeWordmark = ({ size = 18, onDark }) => (
  <span style={{ fontWeight: 900, fontSize: size, fontFamily: "Georgia, serif", background: onDark ? "linear-gradient(135deg,#fff,var(--gj-yellow))" : "linear-gradient(135deg,#19A757,#0A807F)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>Yaye</span>
);
const IaBadge = () => <span style={{ background: "var(--gj-yellow)", color: "var(--gj-teal-deep)", fontSize: 9, fontWeight: 900, padding: "2px 7px", borderRadius: 999, letterSpacing: ".4px" }}>IA</span>;
const TypingDots = () => (
...
const YayeBubbles = () => (
  <YBlock title="Bulles de conversation" sub="Message Yaye · message utilisateur · sources · jour">
    <div style={{ background: "#F5FAF8", borderRadius: 14, padding: 16, display: "flex", flexDirection: "column", gap: 8, maxWidth: 480 }}>
      ...
      <div style={{ alignSelf: "flex-start", maxWidth: "84%" }}>
        ...
        <span style={{ fontSize: 10.5, color: "var(--gj-grey)", marginTop: 3, display: "inline-flex", alignItems: "center", gap: 4 }}><svg className="gj-icon" style={{ width: 11, height: 11 }}><use href="#i-document" /></svg>basé sur ton profil + 142 offres</span>
      </div>
  ...
  <YBlock title="Réponses rapides · insight · nudge · saisie" sub="Suggestions cliquables, encart d'analyse, incitation inline, barre de saisie">
    ...
    <YDemo label="Encart d'analyse (insight)" col>
      <div style={{ display: "flex", gap: 11, background: "linear-gradient(135deg, var(--gj-teal-soft), #fff)", border: "1px solid var(--gj-teal)", borderRadius: 12, padding: "13px 15px", maxWidth: 460 }}>
        <YayeAvatar size={32} />
        <div style={{ flex: 1 }}><div style={{ fontSize: 12.5, fontWeight: 800, color: "var(--gj-teal-deep)" }}><span style={{ fontFamily: "Georgia, serif", fontSize: 14 }}>Le conseil de Yaye</span></div><div style={{ fontSize: 12.5, color: yInk, marginTop: 4, lineHeight: 1.45 }}>Ce stage correspond à ton objectif maraîchage et reste proche de Tambacounda — pense à joindre ton attestation de scolarité.</div></div>
      </div>
    </YDemo>
    ...
    <YDemo label="Barre de saisie" col>
      <div style={{ display: "flex", alignItems: "center", gap: 8, maxWidth: 460 }}>
        <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8, border: "1.5px solid var(--gj-line)", borderRadius: 999, padding: "0 8px 0 16px", minHeight: 46, background: "var(--gj-bg)" }}>
          <span style={{ flex: 1, fontSize: 14, color: "var(--gj-grey-2)" }}>Écris ou parle à Yaye…</span>
          <button style={{ width: 34, height: 34, borderRadius: "50%", border: 0, background: "transparent", color: "var(--gj-teal-deep)", cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center" }} aria-label="Micro"><svg className="gj-icon" style={{ width: 18, height: 18 }}><use href="#i-play" /></svg></button>
        </div>
    ...
    <YDemo label="Bouton flottant (FAB) & suggestions d'accueil">
      <button style={{ width: 56, height: 56, borderRadius: "50%", border: 0, background: "linear-gradient(135deg, #19A757 0%, #0A807F 100%)", color: "#fff", cursor: "pointer", boxShadow: "0 8px 22px rgba(10,128,127,.4)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontFamily: "Georgia, serif", fontSize: 26, fontWeight: 900 }}>Y</button>
...
const OPP_TYPES = {
  emploi:      { label: "Emploi",       icon: "i-employment", c: "#1A4ED8", soft: "#EAF0FD" },
  stage:       { label: "Stage",        icon: "i-target",     c: "#0A807F", soft: "#E4F2F0" },
  alternance:  { label: "Alternance",   icon: "i-learning",   c: "#7C3AED", soft: "#F1EAFB" },
  formation:   { label: "Formation",    icon: "i-resources",  c: "#15803D", soft: "#E7F4EC" },
  bourse:      { label: "Bourse",       icon: "i-funding",    c: "#B0820B", soft: "#FBF3DE" },
  financement: { label: "Financement",  icon: "i-funding",    c: "#C2410C", soft: "#FCEEE6" },
  volontariat: { label: "Volontariat",  icon: "i-heart",      c: "#0E7490", soft: "#E3F1F5" },
  concours:    { label: "Concours",     icon: "i-bolt",       c: "#BE185D", soft: "#FBE8F0" },
  mentorat:    { label: "Mentorat",     icon: "i-users",      c: "#4F46E5", soft: "#ECEBFC" },
  atelier:     { label: "Atelier",      icon: "i-calendar",   c: "#9333EA", soft: "#F3E9FC" },
};
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "13px 14px", display: "flex", flexDirection: "column", gap: 11, flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 9, background: "var(--gj-teal-soft)", borderRadius: 10, padding: "9px 11px" }}>
            <span style={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg,#19A757,#0A807F)", color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontFamily: "Georgia,serif", fontWeight: 900, fontSize: 12 }}>Y</span>
  ...
  <YBlock title="Dépôt de dossier — tous les états" sub="Vide · survol/glisser · téléversement · succès · erreur · vérifié — couvre toutes les éventualités">
    ...
    <YDemo label="États d'un fichier" col>
      ...
      <FileRow icon="i-document" bg="var(--gj-blue-soft)" color="var(--gj-blue-ink)" name="CV_Awa_Diop.pdf" meta="Téléversement… 1,2 Mo / 2,4 Mo"
        right={<div style={{ width: 90, display: "flex", flexDirection: "column", gap: 4 }}><div style={{ height: 6, background: "var(--gj-bg)", borderRadius: 3, overflow: "hidden" }}><div style={{ height: "100%", width: "52%", background: "var(--gj-blue)", borderRadius: 3 }} /></div><span style={{ fontSize: 10, color: "var(--gj-grey)", textAlign: "right", fontWeight: 700 }}>52%</span></div>} />
      {/* success */}
      ...
      <FileRow icon="i-document" bg="var(--gj-teal-soft)" color="var(--gj-teal-deep)" border="var(--gj-teal)" name="Diplôme_Licence.pdf" meta="Vérifié par Yaye · lisible & valide"
        right={<span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 800, color: "var(--gj-teal-deep)", background: "var(--gj-teal-soft)", padding: "4px 10px", borderRadius: 999 }}><span style={{ fontFamily: "Georgia,serif", fontWeight: 900 }}>Y</span>Vérifié</span>} />
      {/* error format */}
