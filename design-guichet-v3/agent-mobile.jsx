...
const MobAgentMore = ({ nav }) => {
  const items = [["benef", "i-users", "Bénéficiaires", "1 284 jeunes"], ["publish", "i-employment", "Publications", "Ateliers & offres"], ["rdv", "i-calendar", "Agenda & RDV", "5 aujourd'hui"], ["settings", "i-settings", "Paramètres", "Compte & notifications"]];
  return (
    <PhoneFrame>
  ...
  <PhoneFrame>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10, background: "var(--gj-bg)" }}>
      ...
      {A_PUBLICATIONS.map((p) => {
        ...
        const tone = { teal: "var(--gj-teal-deep)", blue: "var(--gj-blue-ink)", yellow: "var(--gj-yellow-ink)", green: "var(--gj-green-ink)" }[p.tone];
        const toneSoft = { teal: "var(--gj-teal-soft)", blue: "var(--gj-blue-soft)", yellow: "var(--gj-yellow-soft)", green: "var(--gj-green-soft)" }[p.tone];
        return (
          <div key={p.id} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 13, padding: 13 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              <span style={{ fontSize: 9.5, fontWeight: 800, color: tone, background: toneSoft, padding: "2px 8px", borderRadius: 999, textTransform: "uppercase", letterSpacing: ".3px" }}>{p.kind}</span>
              <span style={{ fontSize: 9.5, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: st.soft, color: st.ink, marginLeft: "auto" }}>{p.status}</span>
            </div>
          ...
          {A_RDV.map((r, i) => {
            ...
            return (
              <div key={r.id} style={{ display: "flex", gap: 12, padding: "13px 14px", borderBottom: i < A_RDV.length - 1 ? "1px solid var(--gj-line)" : 0, alignItems: "center", background: atelier ? "var(--gj-yellow-soft)" : "#fff" }}>
                <div style={{ width: 46, flexShrink: 0, textAlign: "center" }}><div style={{ fontSize: 14, fontWeight: 900, color: "var(--gj-ink)" }}>{r.time}</div><div style={{ fontSize: 9.5, color: "var(--gj-grey-2)" }}>{r.dur}</div></div>
