      ...
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: 20, marginTop: 18, alignItems: "start" }}>
          ...
          <div style={{ ...aCard, position: "sticky", top: 0 }}>
            <div style={{ position: "relative", aspectRatio: "1 / 1", borderRadius: 14, overflow: "hidden", background: "#0E1513" }}>
              {/* simulated camera */}
              <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 40%, #1d2b27, #0E1513)" }} />
        ...
        {A_PUBLICATIONS.map((p) => {
          ...
          return (
            <div key={p.id} style={{ ...aCard, padding: 16, display: "flex", alignItems: "center", gap: 14 }}>
              <span style={{ fontSize: 10, fontWeight: 800, color: tone, background: toneSoft, padding: "4px 10px", borderRadius: 999, textTransform: "uppercase", letterSpacing: ".3px", flexShrink: 0 }}>{p.kind}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
            ...
            return (
              <div key={r.id} style={{ display: "flex", gap: 16, padding: "16px 18px", borderBottom: i < A_RDV.length - 1 ? "1px solid var(--gj-line)" : 0, alignItems: "center", background: atelier ? "var(--gj-yellow-soft)" : "#fff" }}>
                <div style={{ width: 58, flexShrink: 0, textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 10.5, color: "var(--gj-grey-2)" }}>{r.dur}</div>
                </div>
...
const WebAgent = ({ view = "home" }) => {
  ...
  const titles = {
    ...
    checkin: ["Check-in présence", "Atelier en cours"], publish: ["Publications", "Gérer"],
