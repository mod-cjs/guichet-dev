...
const MobileMyResa = ({ nav = () => {} }) => {
  const tabs = [["all", "Toutes"], ["attente", "En attente"], ["passee", "Passées"]];
  return (
    <PhoneFrame>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid var(--gj-line)", padding: "0 14px 12px" }}>
        <div style={{ display: "flex", gap: 6, background: "var(--gj-bg)", borderRadius: 11, padding: 4 }}>
          {tabs.map(([id, label], i) => (
            <span key={id} style={{ flex: 1, textAlign: "center", padding: "10px 0", borderRadius: 8, fontSize: 13, fontWeight: 800, background: i === 0 ? "#fff" : "transparent", color: i === 0 ? "var(--gj-teal-deep)" : "var(--gj-grey)", boxShadow: i === 0 ? "var(--gj-shadow-sm)" : "none" }}>{label}</span>
          ))}
        </div>
      </div>
