      ...
      <div style={{ padding: 15, display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
        ...
        <div style={{ marginTop: "auto", paddingTop: 12, display: "flex", alignItems: "center", gap: 10, borderTop: "1px solid var(--gj-line)" }}>
          {res.lang.includes("Wolof") && (
            <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 10.5, fontWeight: 800, color: "var(--gj-teal-deep)", background: "var(--gj-teal-soft)", padding: "3px 8px", borderRadius: 999 }}>
    ...
    <div style={{ maxWidth: 1080, margin: "0 auto", width: "100%", display: "flex", flexDirection: "column", gap: 26 }}>
      ...
      <section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {featured.map((r) => <ResCard key={r.id} res={r} onOpen={() => onOpen && onOpen(r)} />)}
        </div>
      </section>
      ...
      <section>
        ...
        <div style={{ background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 14, overflow: "hidden" }}>
          {top.map((r, i) => <div key={r.id} style={{ borderBottom: i === top.length - 1 ? 0 : undefined }}><ResRow res={r} rank={i + 1} onOpen={() => onOpen && onOpen(r)} /></div>)}
        </div>
      </section>
    </div>
  );
  ...
  return (
    <aside style={{ width: 230, flexShrink: 0, alignSelf: "flex-start" }}>
      <div style={{ background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 14, padding: 16 }}>
        ...
        <div>
          <div style={{ fontSize: 10.5, fontWeight: 800, color: "var(--gj-grey)", textTransform: "uppercase", letterSpacing: ".4px", marginBottom: 10 }}>Langue</div>
  ...
  return (
    <div style={{ display: "flex", gap: 22, alignItems: "flex-start" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {list.map((r) => <ResCard key={r.id} res={r} onOpen={() => onOpen && onOpen(r)} />)}
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 26, alignItems: "start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
          <div style={{ display: "flex", gap: 22 }}>
            ...
            <div>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: t.soft, color: t.ink, padding: "4px 10px", borderRadius: 999, fontSize: 10.5, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".4px" }}>
            ...
            <div style={{ display: "flex", gap: 16, background: "var(--gj-bg)", border: "1.5px solid var(--gj-line)", borderRadius: 14, padding: 20, overflowX: "auto" }}>
              {[0, 1, 2].map((i) => (
                <div key={i} style={{ position: "relative", flexShrink: 0 }}>
                  ...
                  <span style={{ position: "absolute", bottom: -10, left: "50%", transform: "translateX(-50%)", background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 999, padding: "2px 9px", fontSize: 10.5, fontWeight: 800, color: "var(--gj-grey)" }}>p.{i + 1}</span>
                </div>
              ))}
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
              {related.map((r) => <ResCard key={r.id} res={r} onOpen={() => onOpen && onOpen(r)} />)}
            </div>
          </div>
        </div>
...
const ResReaderContent = ({ res, onBack }) => {
  const t = RES_TYPES[res.type];
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", background: "#2A302E" }}>
      {/* toolbar */}
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 18px", background: "#1F2422", color: "#fff", flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        {/* sommaire */}
        <aside style={{ width: 240, background: "#252B29", color: "#fff", flexShrink: 0, overflowY: "auto", padding: "16px 12px" }}>
          <div style={{ fontSize: 10.5, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".4px", opacity: .55, padding: "0 8px 10px" }}>Sommaire</div>
          {RES_TOC.map((s, i) => (
            <div key={s.n} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 10px", borderRadius: 8, background: i === 0 ? "rgba(255,255,255,.1)" : "transparent", cursor: "pointer", marginBottom: 2 }}>
              ...
              <span style={{ fontSize: 10.5, opacity: .45 }}>{s.page}</span>
            </div>
          ))}
        </aside>
