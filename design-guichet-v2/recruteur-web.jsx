        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {R_OFFERS.filter((o) => o.status === "Active").map((o) => (
            <div key={o.id} onClick={() => nav("pipeline")} style={{ display: "flex", alignItems: "center", gap: 13, padding: "12px 0", borderBottom: "1px solid var(--gj-line)", cursor: "pointer" }}>
              ...
              {o.nouveau > 0 && <span style={{ fontSize: 10.5, fontWeight: 800, color: "#fff", background: "var(--gj-red)", padding: "3px 9px", borderRadius: 999 }}>{o.nouveau} nouv.</span>}
            </div>
          ))}
        </div>
      </div>
        ...
        {R_OFFERS.map((o) => {
          ...
          return (
            <div key={o.id} style={{ ...rCard, padding: 16, display: "flex", alignItems: "center", gap: 15 }}>
              <span style={{ width: 46, height: 46, borderRadius: 11, flexShrink: 0, background: o.status === "Clôturée" ? "var(--gj-bg)" : "var(--gj-blue-soft)", color: o.status === "Clôturée" ? "var(--gj-grey)" : "var(--gj-blue-ink)", display: "inline-flex", alignItems: "center", justifyContent: "center" }}><svg className="gj-icon" style={{ width: 22, height: 22 }}><use href="#i-employment" /></svg></span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 15, fontWeight: 800, color: "var(--gj-ink)" }}>{o.title}</span>
                  <span style={{ fontSize: 10.5, fontWeight: 800, padding: "3px 10px", borderRadius: 999, background: st.soft, color: st.ink }}>{o.status}</span>
                </div>
              ...
              {o.status === "Active" && <button onClick={() => nav("pipeline")} style={{ background: "var(--gj-blue)", color: "#fff", border: 0, padding: "9px 15px", borderRadius: 9, fontWeight: 800, fontSize: 12.5, cursor: "pointer", fontFamily: "inherit", flexShrink: 0, position: "relative" }}>Voir candidatures{o.nouveau > 0 && <span style={{ position: "absolute", top: -7, right: -7, minWidth: 18, height: 18, borderRadius: 999, background: "var(--gj-red)", color: "#fff", fontSize: 10, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center", border: "2px solid #fff", padding: "0 4px" }}>{o.nouveau}</span>}</button>}
    ...
    <div style={{ padding: "22px 28px 40px", overflowY: "auto", flex: 1 }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 18 }}>
          ...
          <button style={{ background: "var(--gj-blue)", color: "#fff", border: 0, padding: "12px 24px", borderRadius: 10, fontWeight: 800, fontSize: 14, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 7 }}><svg className="gj-icon" style={{ width: 16, height: 16 }}><use href="#i-check" /></svg>Soumettre pour validation</button>
        </div>
      </div>
    </div>
  );
...
const PipeCard = ({ c, onOpen }) => (
  <div onClick={onOpen} draggable style={{ background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 11, padding: 12, cursor: "pointer", display: "flex", flexDirection: "column", gap: 9, boxShadow: "var(--gj-shadow-sm)" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 800, color: "var(--gj-ink)" }}>{c.name}</div>
        <div style={{ fontSize: 10.5, color: "var(--gj-grey)" }}>{c.age} ans · {c.commune}</div>
      </div>
    ...
    <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
      {c.skills.slice(0, 3).map((s) => <span key={s} style={{ fontSize: 10, fontWeight: 700, color: "var(--gj-grey)", background: "var(--gj-bg)", border: "1px solid var(--gj-line)", padding: "2px 8px", borderRadius: 999 }}>{s}</span>)}
    </div>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid var(--gj-line)", paddingTop: 9 }}>
      {matchBadge(c.match)}
      <span style={{ fontSize: 10.5, color: "var(--gj-grey-2)" }}>{c.applied}</span>
    </div>
  </div>
);
