...
describe('SSOApiClient', () => {
  ...
  it('provisionBulk chunke en lots de 500', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(3) // 500 + 500 + 234
    const sentSizes = fetchMock.mock.calls.map(([, init]) => {
  ...
  it('provisionBulk agrège les results de tous les batches', async () => {
    fetchMock
      .mockResolvedValueOnce(jsonResponse(200, {
        results: [{ uuid: 'u1', cjs_uid: 'u1', drupal_uid: 1, created: true }],
      }))
      .mockResolvedValueOnce(jsonResponse(200, {
        results: [{ uuid: 'u2', cjs_uid: 'u2', drupal_uid: 2, created: false }],
      }))
