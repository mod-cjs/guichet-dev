...
- **Recommandation — un seul moteur, deux modes d'accès** : le **raisonnement** se fait **toujours par traversée du graphe** ([02 §0](./02-knowledge-graph-neo4j.md), invariant « moteur unique »). `query_knowledge_graph` = la traversée **en direct** (réactif : « trouve-moi un financement… ») ; `get_recommendations` = **la même traversée pré-calculée**, lue depuis `RecommandationIA` (proactif, sans LLM). Le **pipeline de scoring** (batch + à la création d'opportunité) implémente `src/lib/ia/recommandation.ts` (stub aujourd'hui) : il **orchestre le graphe et écrit le cache** — il **n'invente aucun score**. Les scores **naissent en Prisma** (read-model), le raisonnement reste dans le graphe.
