...
| Sujet | État | Ticket |
|---|---|---|
| Règle pare-feu Prometheus non persistée | **Corrigé en pratique, pas en configuration durable.** Le bind Prometheus (`0.0.0.0:9090`) exige une règle `iptables ACCEPT` scopée au sous-réseau Docker `observabilite` pour rester protégé du public — ajoutée manuellement le 20/08, mais **non versionnée** : ne survit pas à un redémarrage serveur ni à un flush iptables. Voir `docs/netdata.md` (section GUIC-714) pour la commande exacte. Persister durablement (mécanisme de pare-feu déjà en place sur le serveur, à investiguer) reste à faire. | GUIC-714 |
