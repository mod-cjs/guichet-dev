...
| Accès dashboards (Cloudflare Tunnel + Access) | Schéma tranché, préparé (`docker-compose.cloudflared.yml`, `docs/cloudflare-acces-dashboards.md`) — reste les étapes manuelles côté Cloudflare (créer le tunnel, politiques d'accès) puis le déploiement serveur, **non vérifié en réel** | GUIC-575 |
