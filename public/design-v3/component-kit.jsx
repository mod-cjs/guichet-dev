...
const Badges = () => (
  <Block title="Pastilles de statut & badges" sub="Couleurs sémantiques : vert succès · jaune attente · bleu info · rouge alerte · gris neutre">
    ...
    <Demo label="Compteurs & notifications">
      ...
      <span style={{ fontSize: 9.5, fontWeight: 800, color: "var(--gj-teal-deep)", background: "var(--gj-teal-soft)", padding: "3px 9px", borderRadius: 999, textTransform: "uppercase", letterSpacing: ".3px" }}>Membre CJS</span>
...
const Avatars = () => (
  <Block title="Avatars" sub="Initiales sur dégradé — teal (jeune), bleu (recruteur), doré (agent/admin)">
    <Demo>
      ...
      <span style={{ position: "relative", display: "inline-flex" }}>{av("CN", 48, "linear-gradient(135deg, var(--gj-yellow), #E0A93B)")}<span style={{ position: "absolute", right: 0, bottom: 0, width: 13, height: 13, borderRadius: "50%", background: "var(--gj-green)", border: "2px solid #fff" }} /></span>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 6, background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 12, padding: 5 }}>
            {[["all", "Toutes", 6], ["cours", "En cours", 4], ["clos", "Clôt.", 2]].map(([id, label, n]) => {
              const on = id === v;
              return <span key={id} onClick={() => setV(id)} style={{ flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "9px 0", borderRadius: 8, fontSize: 12.5, fontWeight: 800, cursor: "pointer", background: on ? "var(--gj-teal-deep)" : "transparent", color: on ? "#fff" : "var(--gj-grey)" }}>{label}<span style={{ fontSize: 10, fontWeight: 800, background: on ? "rgba(255,255,255,.22)" : "var(--gj-bg)", color: on ? "#fff" : "var(--gj-grey)", padding: "1px 6px", borderRadius: 999 }}>{n}</span></span>;
            })}
          </div>
        </div>
        <div>
