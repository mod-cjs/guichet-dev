...
// =====================================================================
const QRGlyph = ({ size = 160, plain = false, fluid = false }) => {
  ...
  return (
    <svg width={fluid ? "100%" : size} height={fluid ? "100%" : size} viewBox={`0 0 ${size} ${size}`} style={{ display: "block", background: "#fff", borderRadius: 8 }}>
      <rect x="0" y="0" width={size} height={size} fill="#fff" />
      {[...filled].map((k, i) => {
        const [x, y] = k.split(",").map(Number);
        return <rect key={i} x={x * cell} y={y * cell} width={cell} height={cell} fill={plain ? "#111" : "#202020"} />;
      })}
      {!plain && <rect x={(cx - 2.5) * cell} y={(cy - 2.5) * cell} width={5 * cell} height={5 * cell} rx={cell * 1.2} fill="#fff" stroke="#202020" strokeWidth={cell * 0.6} />}
...
// =====================================================================
const EMVChip = ({ w = 38 }) => (
  <div style={{ width: w, height: w * 0.76, borderRadius: w * 0.16, background: "linear-gradient(135deg, #f8a309 0%, #C97F03 45%, #7A4E02 100%)", position: "relative", flexShrink: 0, boxShadow: "inset 0 0 0 1px rgba(0,0,0,.12)", overflow: "hidden" }}>
...
// =====================================================================
// Les barres sont proportionnelles (flex-grow) et non en pixels : la colonne
// code-barres peut alors rétrécir avec la carte au lieu d'écraser la rangée
// d'accès voisine, qui débordait sur 4 lignes.
const Barcode = ({ height = 46, width = "100%" }) => {
  const w = [2, 4, 2, 3, 5, 2, 2, 4, 2, 3, 2, 5, 3, 2, 2, 4, 2, 3, 5, 2, 2, 3, 4, 2, 2, 5, 2, 3, 2, 4, 2, 2, 3];
  return (
    <div style={{ display: "flex", alignItems: "stretch", gap: 1.5, height, width, minWidth: 0 }}>
      {w.map((x, i) => <span key={i} style={{ flex: x + " 1 0", background: "#111", minWidth: 0.5 }} />)}
    </div>
  );
};
...
const CardShell = ({ children, maxWidth = 420, folded = false, foldText }) => (
  <div style={{
    ...
    {folded && (
      <React.Fragment>
        <div style={{ position: "absolute", top: 0, right: 0, width: "24%", aspectRatio: "1 / 1", background: "linear-gradient(135deg, var(--gj-yellow), #C97F03)", clipPath: "polygon(100% 0, 0 0, 100% 100%)", zIndex: 2 }} />
        {foldText && (
          <div style={{ position: "absolute", top: "8%", right: "5%", textAlign: "right", lineHeight: 1.05, zIndex: 3 }}>
            <div style={{ fontSize: "clamp(11px,2.5%,12px)", fontWeight: 800, letterSpacing: ".6px", color: "var(--gj-ink)" }}>CARTE</div>
  ...
  <CardShell maxWidth={maxWidth} folded foldText="2026">
    <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "clamp(14px,4.4%,20px)", paddingLeft: "clamp(18px,5.4%,24px)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "clamp(8px,2.6%,11px)" }}>
        ...
        <span style={{ fontSize: "clamp(11px,2.4%,12px)", fontWeight: 800, color: "var(--gj-teal-deep)", letterSpacing: ".6px", textTransform: "uppercase" }}>Bokk · Jàng · Liguéey</span>
      </div>
      {/* body */}
      <div style={{ display: "flex", gap: "clamp(13px,4.2%,18px)", alignItems: "center" }}>
        {/* Le QR est dimensionné en POURCENTAGE de la largeur de carte : la
            feuille a un ratio fixe de 1,585:1, donc sa hauteur suit sa largeur.
            Un QR en pixels fixes débordait dès que la carte passait sous
            380 px — c'est ce qui rognait le pied de carte. */}
        <div style={{ background: "var(--gj-teal-soft)", border: "1px solid var(--gj-line)", borderRadius: 10, padding: "clamp(5px,1.6%,8px)", flexShrink: 0, width: "clamp(74px,29%,124px)", aspectRatio: "1 / 1" }}>
          <QRGlyph size={124} plain fluid />
        </div>
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: "clamp(11px,2.8%,12px)", fontWeight: 800, color: "var(--gj-teal-deep)", letterSpacing: ".4px", textTransform: "uppercase" }}>
      ...
      <div style={{ borderTop: "1px solid var(--gj-line)", paddingTop: "clamp(8px,2.6%,11px)", display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 10 }}>
        ...
        <span style={{ textAlign: "right" }}>
          <span style={{ display: "block", fontSize: 11, fontWeight: 800, color: "var(--gj-grey)", letterSpacing: ".6px", textTransform: "uppercase" }}>Inscrite le</span>
...
const MyCJSCardBack = ({ maxWidth = 420 }) => {
  const chip = (label) => (
    <span key={label} style={{ border: "1.5px solid var(--gj-line-strong)", borderRadius: 8, padding: "4px clamp(7px,2.4%,10px)", fontSize: "clamp(11px,2.9%,12px)", fontWeight: 700, color: "var(--gj-ink)", whiteSpace: "nowrap" }}>{label}</span>
  );
  return (
    <CardShell maxWidth={maxWidth}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "clamp(14px,4.4%,20px)", paddingLeft: "clamp(18px,5.4%,24px)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          ...
          <span style={{ fontSize: "clamp(11px,2.6%,12px)", fontWeight: 800, color: "var(--gj-grey)", letterSpacing: ".5px", textTransform: "uppercase", flexShrink: 0 }}>Bénéficiaire</span>
        </div>
        {/* accès + code-barres */}
        <div style={{ display: "flex", gap: "clamp(9px,3%,14px)", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: "clamp(11px,3.3%,13px)", fontWeight: 900, color: "var(--gj-ink)", letterSpacing: ".3px", marginBottom: "clamp(5px,1.8%,7px)" }}>CETTE CARTE OUVRE L'ACCÈS À</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "clamp(4px,1.6%,6px)", maxWidth: "100%" }}>
              {["Centres", "Ateliers", "Cours", "Bibliothèque", "Événements"].map(chip)}
            </div>
          </div>
          <div style={{ textAlign: "center", width: "clamp(84px,27%,132px)", flexShrink: 1, minWidth: 0 }}>
            {/* Hauteur dérivée de la LARGEUR via aspect-ratio : un pourcentage de
                hauteur se résoudrait contre un parent de hauteur indéfinie et
                clamp() s'effondrerait à 0, effaçant le code-barres. */}
            <div style={{ width: "100%", aspectRatio: "3.4 / 1", minHeight: 24 }}>
              <Barcode height="100%" />
            </div>
            <div style={{ fontFamily: "ui-monospace, Menlo, monospace", fontSize: 11, fontWeight: 700, color: "var(--gj-ink)", marginTop: 4, letterSpacing: ".3px", whiteSpace: "nowrap" }}>GJ2026 77294A</div>
          </div>
        </div>
        {/* footer */}
        <div style={{ borderTop: "1px solid var(--gj-line)", paddingTop: "clamp(8px,2.6%,11px)", display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 12 }}>
          <div style={{ fontSize: "clamp(11px,2.3%,12px)", color: "var(--gj-grey)", lineHeight: 1.35, maxWidth: "58%", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          ...
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            ...
            <div style={{ fontSize: "clamp(11px,3%,12px)", fontWeight: 800, color: "var(--gj-ink)", marginTop: 2, display: "inline-flex", alignItems: "center", gap: 4 }}>
