...
const AdmBottomNav = ({ active = "home", nav = () => {} }) => {
  ...
  return (
    <div style={{ display: "flex", borderTop: "1px solid rgba(255,255,255,.1)", background: "#162c5e", flexShrink: 0, paddingBottom: 6 }}>
      {items.map((it) => {
        ...
        return (
          <button key={it.id} onClick={() => nav(it.id)} style={{ flex: 1, background: "transparent", border: 0, cursor: "pointer", fontFamily: "inherit", padding: "9px 0 5px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, color: on ? "var(--gj-yellow)" : "rgba(255,255,255,.55)" }}>
            <span style={{ position: "relative" }}>
              ...
              {it.badge && <span style={{ position: "absolute", top: -5, right: -9, minWidth: 16, height: 16, borderRadius: 999, background: "var(--gj-red)", color: "#fff", fontSize: 11, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center", border: "2px solid #162c5e", padding: "0 3px" }}>{it.badge}</span>}
            </span>
...
const AdmHeader = ({ title, sub, onBack }) => (
  <div style={{ background: "#162c5e", color: "#fff", padding: "13px 15px", display: "flex", alignItems: "center", gap: 11, flexShrink: 0 }}>
    ...
    <span style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg, var(--gj-yellow), #C97F03)", color: "#162c5e", display: "inline-flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 12, flexShrink: 0 }}>{ADMIN.initials}</span>
  </div>
);

const amAvatar = (init, size = 36, tone) => {
  const g = { teal: ["var(--gj-teal)", "var(--gj-teal-deep)"], blue: ["var(--gj-blue)", "var(--gj-blue-ink)"], yellow: ["var(--gj-yellow)", "#C97F03"], red: ["var(--gj-red)", "var(--gj-red-ink)"], grey: ["var(--gj-grey)", "var(--gj-grey-2)"] }[tone] || ["var(--gj-teal)", "var(--gj-teal-deep)"];
  return <span style={{ width: size, height: size, borderRadius: "50%", flexShrink: 0, background: `linear-gradient(135deg, ${g[0]}, ${g[1]})`, color: tone === "yellow" ? "#162c5e" : "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: size * 0.36 }}>{init}</span>;
};
          ...
          {ADM_KPIS.map((k, i) => {
            ...
            return (
              <div key={i} onClick={() => k.urgent && nav("moderation")} style={{ background: "#fff", border: k.urgent ? "1.5px solid var(--gj-yellow)" : "1px solid var(--gj-line)", borderRadius: 13, padding: 13 }}>
                ...
                <div style={{ fontSize: 11, fontWeight: 700, color: "var(--gj-ink)", marginTop: 4 }}>{k.label}</div>
              </div>
            );
          })}
        </div>
          ...
          <div style={{ display: "flex", gap: 11 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-grey)", background: "var(--gj-bg)", padding: "2px 7px", borderRadius: 999, textTransform: "uppercase" }}>{m.kind}</span>
                {m.flag && <span style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-red-ink)", background: "var(--gj-red-soft)", padding: "2px 7px", borderRadius: 999, textTransform: "uppercase" }}>Signalé</span>}
              </div>
            ...
            return (
              <div style={{ background: "linear-gradient(135deg, var(--gj-teal-soft), #fff)", border: "1px solid var(--gj-line)", borderRadius: 10, padding: 11 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ width: 26, height: 26, borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg, #19a657, #027f7e)", color: "#fff", fontFamily: "var(--gj-font-sans)", fontWeight: 900, fontSize: 13, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>Y</span>
                  <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 11.5, fontWeight: 800, color: "var(--gj-ink)" }}>Analyse Yaye <span style={{ background: "var(--gj-yellow)", color: "var(--gj-ink)", fontSize: 11, fontWeight: 900, padding: "1px 5px", borderRadius: 999 }}>IA</span></div></div>
                ...
                <div style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 8, fontSize: 11, fontWeight: 800, color: vd.ink, background: vd.soft, padding: "3px 9px", borderRadius: 999 }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: vd.dot }} />{vd.label}</div>
              </div>
            );
          })()}
        ...
        {ADM_USERS.map((u) => {
          ...
          return (
            <div key={u.id} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 12, padding: 12, display: "flex", alignItems: "center", gap: 11 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: rt.soft, color: rt.ink, flexShrink: 0 }}>{u.role}</span>
            </div>
          );
        })}
      </div>
  ...
  return (
    <PhoneFrame>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 18, background: "var(--gj-bg)" }}>
        {groups.map((g, gi) => (
          <div key={gi}>
            <div style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-grey)", textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 8 }}>{g.t}</div>
        ...
        return (
          <div key={p.id} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 13, padding: 13, display: "flex", flexDirection: "column", gap: 11 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
              ...
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 800, padding: "3px 9px", borderRadius: 999, background: st.soft, color: st.ink, flexShrink: 0 }}><svg className="gj-icon" style={{ width: 11, height: 11 }}><use href={"#" + st.icon} /></svg>{p.statut}</span>
            </div>
            {p.statut === "À vérifier" && (
              <div style={{ display: "flex", gap: 8, borderTop: "1px solid var(--gj-line)", paddingTop: 11 }}>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10, background: "var(--gj-bg)" }}>
      ...
      {ADM_CONTENT.map((ct) => (
        <div key={ct.id} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 12, padding: 12, display: "flex", alignItems: "center", gap: 11 }}>
          <span style={{ width: 32, height: 38, borderRadius: 5, background: "var(--gj-red-soft)", color: "var(--gj-red-ink)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 11, flexShrink: 0 }}>PDF</span>
          ...
          <span style={{ fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: ct.statut === "Publié" ? "var(--gj-green-soft)" : "var(--gj-bg)", color: ct.statut === "Publié" ? "var(--gj-green-ink)" : "var(--gj-grey)", flexShrink: 0 }}>{ct.statut}</span>
        </div>
      ))}
    </div>
    <AdmBottomNav active="more" nav={nav} />
      ...
      {ADM_ROLES.map((r, i) => (
        <div key={i} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 13, padding: 13 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0, background: i === 4 ? "linear-gradient(135deg, var(--gj-yellow), #C97F03)" : "var(--gj-teal-soft)", color: i === 4 ? "#162c5e" : "var(--gj-teal-deep)", display: "inline-flex", alignItems: "center", justifyContent: "center" }}><svg className="gj-icon" style={{ width: 19, height: 19 }}><use href={"#" + (i === 4 ? "i-shield" : "i-users")} /></svg></span>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>{r.perms.map((p) => <span key={p} style={{ fontSize: 11, fontWeight: 700, color: "var(--gj-grey)", background: "var(--gj-bg)", border: "1px solid var(--gj-line)", padding: "3px 9px", borderRadius: 999 }}>{p}</span>)}</div>
        </div>
      ))}
    </div>
    <AdmBottomNav active="more" nav={nav} />
