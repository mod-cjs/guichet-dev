  ...
  return (
    <div onClick={onOpen} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 14, padding: 13, display: "flex", gap: 13, cursor: "pointer" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 5 }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 4, alignSelf: "flex-start", background: t.soft, color: t.ink, padding: "2px 8px", borderRadius: 999, fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".3px" }}>
    ...
    <PhoneFrame>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: "var(--gj-bg)" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, var(--gj-teal-deep) 0%, var(--gj-ink-teal) 100%)", color: "#fff", padding: "16px 16px 18px", position: "relative", overflow: "hidden" }}>
          <span style={{ position: "absolute", right: -40, top: -50, width: 170, height: 170, background: "radial-gradient(circle, rgba(248,163,9,.16), transparent 60%)" }} />
        ...
        <div style={{ padding: "16px 14px", display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                return (
                  <div key={c.id} onClick={() => nav("list")} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 13, padding: 13, display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: "var(--gj-grey)", marginTop: 2 }}>{c.n} ressources</div>
                    </div>
                  </div>
                );
              })}
          ...
          <div>
            ...
            <BookShelf>
              {featured.map((r) => <div key={r.id} onClick={() => nav("detail")}><BookCover res={r} w={128} /></div>)}
            </BookShelf>
          </div>
        </div>
      </div>
    ...
    <PhoneFrame>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 14, background: "var(--gj-bg)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "18px 12px" }}>
          {RESOURCES.map((r) => <div key={r.id} onClick={() => nav("detail")}><BookCover res={r} w={106} /></div>)}
        </div>
      </div>
      <BottomNav active="explore" />
    </PhoneFrame>
    ...
    <PhoneFrame>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 18, background: "var(--gj-bg)" }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 12 }}>
          <BookCover res={res} w={150} caption={false} />
          <div>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: t.soft, color: t.ink, padding: "4px 11px", borderRadius: 999, fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".4px" }}>
...
const MobileResReader = ({ nav = () => {} }) => {
  ...
  const t = RES_TYPES[res.type];
  return (
    <PhoneFrame bg="#202020">
      {/* top bar */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "#0E1C3C", color: "#fff", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, opacity: .6 }}>{res.format} · page 3 / 24</div>
        </div>
      ...
      {/* pages */}
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 0 16px", display: "flex", flexDirection: "column", alignItems: "center", gap: 18, background: "#202020" }}>
        {[3, 4].map((p) => (
      ...
      {/* bottom toolbar */}
      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 14px", background: "#0E1C3C", color: "#fff", flexShrink: 0 }}>
