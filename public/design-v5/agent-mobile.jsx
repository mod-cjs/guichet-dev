...
const AgentMobHeader = ({ title, sub, onBack }) => (
  <div style={{ background: "var(--gj-ink-teal)", color: "#fff", padding: "12px 14px", display: "flex", alignItems: "center", gap: 11, flexShrink: 0 }}>
    ...
    <span style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg, var(--gj-yellow), #C97F03)", color: "var(--gj-ink-teal)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 12, flexShrink: 0 }}>{AGENT.initials}</span>
  </div>
);
            ...
            <div style={{ display: "flex", gap: 11 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
                  {mAvatar(r.init, 30)}<span style={{ fontSize: 14, fontWeight: 900, color: "var(--gj-ink)" }}>{r.who}</span>
                </div>
                <div style={{ fontSize: 12.5, fontWeight: 800, color: k.ink, display: "inline-flex", alignItems: "center", gap: 5 }}><svg className="gj-icon" style={{ width: 13, height: 13 }}><use href={"#" + k.icon} /></svg>{r.res}</div>
                <div style={{ display: "flex", gap: 12, marginTop: 7, flexWrap: "wrap" }}>{mMeta("i-calendar", r.date)}{mMeta("i-clock", r.slot)}</div>
                <div style={{ fontSize: 12, color: "var(--gj-grey)", marginTop: 6, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>« {r.motif} »</div>
              </div>
            </div>
  ...
  return (
    <PhoneFrame>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: "var(--gj-bg)", display: "flex", flexDirection: "column" }}>
        {/* scanner */}
        <div style={{ position: "relative", background: "#162c5e", aspectRatio: "1 / 1", flexShrink: 0, overflow: "hidden" }}>
          <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 42%, #0E1C3C, #162c5e)" }} />
            ...
            {A_ATTENDEES.map((a) => (
              <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 11, padding: "9px 11px" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--gj-grey)" }}>{a.in ? `Pointé à ${a.at}` : a.commune}</div>
                </div>
                {a.in
          ...
          {t.msgs.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: m.me ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "78%", background: m.me ? "var(--gj-teal-deep)" : "#fff", color: m.me ? "#fff" : "var(--gj-ink)", border: m.me ? 0 : "1px solid var(--gj-line)", borderRadius: m.me ? "14px 14px 4px 14px" : "14px 14px 14px 4px", padding: "10px 13px", fontSize: 13.5, lineHeight: 1.45 }}>
                {m.t}<div style={{ fontSize: 11, opacity: .6, marginTop: 4, textAlign: "right" }}>{m.time}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 14, background: "var(--gj-bg)" }}>
          <div style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 14, padding: 15, display: "flex", gap: 14, alignItems: "center" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 800, padding: "2px 9px", borderRadius: 999, background: "var(--gj-green-soft)", color: "var(--gj-green-ink)", marginTop: 5, display: "inline-block" }}>{b.statut}</span>
            </div>
...
const MobAgentApp = ({ start = "home" }) => {
  ...
  if (s === "more") return <MobAgentMore nav={nav} />;
  if (s === "library") return <MobAgentLibrary nav={nav} />;
  if (s === "publish") return <MobAgentPublish nav={nav} />;
  if (s === "rdv") return <MobAgentAgenda nav={nav} />;
  if (s === "settings") return <MobAgentSettings nav={nav} />;
  return <MobAgentHome nav={nav} />;
};

// ===== « PLUS » =====
const MobAgentMore = ({ nav }) => {
  const items = [["benef", "i-users", "Bénéficiaires", "1 284 jeunes"], ["library", "i-resources", "Bibliothèque", "42 livres · 2 retards"], ["publish", "i-employment", "Publications", "Ateliers & offres"], ["rdv", "i-calendar", "Agenda & RDV", "5 aujourd'hui"], ["settings", "i-settings", "Paramètres", "Compte & notifications"]];
  return (
    <PhoneFrame>
      <AgentMobHeader title="Plus" sub="Toutes les sections" />
      ...
      <AgentBottomNav active="more" nav={nav} />
    </PhoneFrame>
  );
};

// ===== PUBLICATIONS =====
const MobAgentPublish = ({ nav }) => (
  <PhoneFrame>
    <AgentMobHeader title="Publications" onBack={() => nav("more")} />
    <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10, background: "var(--gj-bg)" }}>
      ...
      {A_PUBLICATIONS.map((p) => {
        ...
        return (
          <div key={p.id} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 13, padding: 13 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: tone, background: toneSoft, padding: "2px 8px", borderRadius: 999, textTransform: "uppercase", letterSpacing: ".3px" }}>{p.kind}</span>
              <span style={{ fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: st.soft, color: st.ink, marginLeft: "auto" }}>{p.status}</span>
            </div>
            <div style={{ fontSize: 14, fontWeight: 800, color: "var(--gj-ink)", lineHeight: 1.25 }}>{p.title}</div>
            <div style={{ fontSize: 11.5, color: "var(--gj-grey)", marginTop: 3 }}>{p.date}{p.cap ? ` · ${p.appli}/${p.cap} places` : p.appli ? ` · ${p.appli} candidatures` : ""}</div>
          </div>
        );
      })}
    </div>
...
const MobAgentAgenda = ({ nav }) => {
  ...
  return (
    <PhoneFrame>
      <AgentMobHeader title="Agenda" sub="Jeudi 8 juin · 5 RDV" onBack={() => nav("more")} />
      <div style={{ flex: 1, overflowY: "auto", padding: 14, background: "var(--gj-bg)" }}>
        <div style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 14, overflow: "hidden" }}>
          {A_RDV.map((r, i) => {
            const tone = statusTone[r.status] || statusTone["confirmé"];
            const atelier = r.status === "atelier";
            return (
              <div key={r.id} style={{ display: "flex", gap: 12, padding: "13px 14px", borderBottom: i < A_RDV.length - 1 ? "1px solid var(--gj-line)" : 0, alignItems: "center", background: atelier ? "var(--gj-yellow-soft)" : "#fff" }}>
                <div style={{ width: 46, flexShrink: 0, textAlign: "center" }}><div style={{ fontSize: 14, fontWeight: 900, color: "var(--gj-ink)" }}>{r.time}</div><div style={{ fontSize: 11, color: "var(--gj-grey-2)" }}>{r.dur}</div></div>
                {mAvatar(r.init, 36, atelier)}
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 800, color: "var(--gj-ink)" }}>{r.who}</div><div style={{ fontSize: 11, color: "var(--gj-grey)", marginTop: 1 }}>{r.type} · {r.topic}</div></div>
              </div>
            );
          })}
        </div>
...
// ===== PARAMÈTRES =====
const MobAgentSettings = ({ nav }) => {
  const sw = (on) => <span style={{ width: 44, height: 26, borderRadius: 999, background: on ? "var(--gj-teal)" : "var(--gj-line-strong)", position: "relative", flexShrink: 0 }}><span style={{ position: "absolute", top: 3, left: on ? 21 : 3, width: 20, height: 20, borderRadius: "50%", background: "#fff" }} /></span>;
  return (
    <PhoneFrame>
      <AgentMobHeader title="Paramètres" onBack={() => nav("more")} />
      <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 14, background: "var(--gj-bg)" }}>
        <div style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 14, padding: 15, display: "flex", alignItems: "center", gap: 13 }}>
          {mAvatar(AGENT.initials, 52, true)}
          <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 15, fontWeight: 900, color: "var(--gj-ink)" }}>{AGENT.name}</div><div style={{ fontSize: 11.5, color: "var(--gj-grey)", marginTop: 1 }}>{AGENT.role} · {AGENT.centre}</div></div>
        </div>
        <div style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 14, padding: "4px 15px" }}>
          {[["Notifications push", true], ["Alertes réservations", true], ["Résumé quotidien par e-mail", false]].map(([l, on], i, arr) => (
...
Object.assign(window, { AgentBottomNav, MobAgentHome, MobAgentResa, MobAgentCheckin, MobAgentMessages, MobAgentBenef, MobAgentApp, MobAgentMore, MobAgentPublish, MobAgentAgenda, MobAgentSettings });
