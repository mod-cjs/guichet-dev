...
const BenefSidebar = ({ active = "home", onNavChange = () => {} }) => {
  ...
  const brandSub = { fontSize: 11, color: "var(--gj-teal-deep)", letterSpacing: ".5px", textTransform: "uppercase", fontWeight: 800 };
  ...
  const userMeta = { fontSize: 11, color: "var(--gj-grey)", marginTop: 2 };

  const sectionH = {
    fontSize: 11, color: "var(--gj-grey)", fontWeight: 800,
  ...
  // CTA réglages d'accessibilité permanent en pied de colonne
  const inclusionFoot = {
    ...
    // La colonne peut dépasser sa hauteur : sans cela ce bloc, seul élément
    // rétractable (overflow:hidden annule sa taille minimale automatique),
    // absorbe tout le débordement et masque le CTA.
    flexShrink: 0,
  };
  ...
  return (
    <aside style={wrap}>
      ...
      <div style={inclusionFoot}>
        <span style={{ position: "absolute", right: -40, top: -40, width: 120, height: 120, background: "radial-gradient(circle, rgba(248,163,9,.25), transparent 60%)", pointerEvents: "none" }} />
        ...
        <button onClick={() => window.gjOpenA11y && window.gjOpenA11y()} style={{
          background: "var(--gj-yellow)", color: "var(--gj-ink)",
          border: 0, padding: "8px 12px", borderRadius: 8, minHeight: 34,
          fontWeight: 800, fontSize: 11.5, cursor: "pointer", fontFamily: "inherit",
          position: "relative",
        }}>Réglages d'accessibilité →</button>
      </div>
    </aside>
  );
};
  ...
  return (
    <div style={wrap}>
      <div style={search}>
        ...
        <kbd style={{ fontFamily: "ui-monospace,monospace", fontSize: 11, background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 4, padding: "1px 5px", color: "var(--gj-grey)" }}>⌘ K</kbd>
      </div>
      <span style={{ flex: 1 }} />
      ...
      <button style={iconBtn} aria-label="Notifications">
        ...
        <span style={{ position: "absolute", top: -4, right: -4, minWidth: 18, height: 18, padding: "0 5px", borderRadius: 999, background: "var(--gj-red)", color: "#fff", fontSize: 11, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center", border: "2px solid #fff" }}>3</span>
      </button>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: "auto", flexShrink: 0 }}>
        <button onClick={onYaye} style={{
          ...
          <span style={{
            ...
            color: "#fff", fontFamily: "var(--gj-font-sans)", fontWeight: 900, fontSize: 14,
            display: "inline-flex", alignItems: "center", justifyContent: "center",
            boxShadow: "0 0 0 1.5px rgba(255,255,255,.3)", flexShrink: 0,
          }}>Y</span>
          <span>Yaye</span>
          <span style={{ background: "var(--gj-yellow)", color: "var(--gj-ink)", fontSize: 11, fontWeight: 800, padding: "1px 6px", borderRadius: 999, letterSpacing: ".3px" }}>IA</span>
        </button>
...
const WebDashHero = () => {
  ...
  const glow = { position: "absolute", right: -50, top: -50, width: 280, height: 280, background: "radial-gradient(circle, rgba(248,163,9,.18), transparent 60%)", pointerEvents: "none" };
  return (
    <section style={hero}>
      <span style={glow} />
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{
            ...
            color: "#fff", fontFamily: "var(--gj-font-sans)", fontWeight: 900, fontSize: 18,
            display: "inline-flex", alignItems: "center", justifyContent: "center",
            boxShadow: "0 0 0 2px rgba(255,255,255,.2)",
          }}>Y</span>
          <div>
            <div>
              <span style={{ fontSize: 14, fontWeight: 900, fontFamily: "var(--gj-font-sans)", background: "linear-gradient(135deg, #fff, var(--gj-yellow))", WebkitBackgroundClip: "text", color: "transparent" }}>Yaye</span>
              <span style={{ background: "var(--gj-yellow)", color: "var(--gj-ink)", fontSize: 11, fontWeight: 800, padding: "1px 6px", borderRadius: 999, marginLeft: 6 }}>IA</span>
            </div>
...
// Résolution de catégorie — partagée (retour design V3).
// Aucun repli silencieux : un libellé sans mot de catégorie donne "neutre".
const GJ_CATS = [
...
const gjCat = (label) => {
  const hit = (GJ_CATS.find(([re]) => re.test(label || "")) || [null, "neutre"])[1];
  return { key: hit, bg: "var(--cat-" + hit + "-soft)", fg: "var(--cat-" + hit + "-ink)", solid: "var(--cat-" + hit + ")" };
};
// « TYPE · J-x » → { cat, j, urgent } : type et urgence sont deux pastilles.
const gjSplitTag = (tag) => {
  const parts = String(tag || "").split("·").map((s) => s.trim()).filter(Boolean);
  const jPart = parts.find((p) => /^j-\d+$/i.test(p) || /urgent|cl[ôo]ture/i.test(p)) || "";
  const catPart = parts.find((p) => p !== jPart) || "";
  const j = /^j-\d+$/i.test(jPart) ? jPart.toUpperCase() : jPart;
  const n = (j.match(/\d+/) || [])[0];
  return { catLabel: catPart, j, urgent: /urgent/i.test(jPart) || (n !== undefined && Number(n) <= 7) };
};

const WebOppCard = ({ tag, tagTone = "cjs", title, org, meta = [], match, cta, width }) => {
  // Code couleur catégorie (retour design V3) : la couleur du badge découle du
  // LIBELLÉ, plus du tagTone — une catégorie = une couleur sur tout le produit.
  const { catLabel, j, urgent } = gjSplitTag(tag);
  const t = gjCat(catLabel);
  const [why, setWhy] = React.useState(false);
  // tuile sectorielle (signalétique)
  const SECT = [
    [/data|num[ée]rique|dev|web|informatique/i, ["i-desktop", "linear-gradient(135deg,#1e35ba,#162c5e)"]],
    [/agro|mara[iî]ch|agri/i, ["i-agriculture", "linear-gradient(135deg,#027f7e,#014B4A)"]],
    [/bourse|mobilit[ée]|master|étude/i, ["i-funding", "linear-gradient(135deg,#f8a309,#C97F03)"]],
    [/animateur|communautaire|social/i, ["i-engagement", "linear-gradient(135deg,#19a657,#0E6234)"]],
    [/formation|bootcamp|atelier/i, ["i-learning", "linear-gradient(135deg,#027f7e,#162c5e)"]],
  ];
  const sf = SECT.find(([re]) => re.test(title || "")) || [null, ["i-employment", "linear-gradient(135deg,var(--gj-teal),var(--gj-teal-deep))"]];
  const [sIcon, sBg] = sf[1];
  return (
    <div style={{
      ...
      <div style={{ position: "relative", height: 54, background: sBg, flexShrink: 0, display: "flex", alignItems: "center", padding: "0 14px", overflow: "hidden" }}>
        <svg className="gj-icon" style={{ position: "absolute", right: -10, bottom: -14, width: 54, height: 54, opacity: .22, color: "#fff" }}><use href={"#" + sIcon} /></svg>
        <svg className="gj-icon" style={{ width: 24, height: 24, color: "#fff", position: "relative" }}><use href={"#" + sIcon} /></svg>
        <span style={{ marginLeft: "auto", position: "relative", width: 24, height: 24, borderRadius: 7, background: "rgba(255,255,255,.94)", color: "var(--gj-ink)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 900 }}>{(org || "?")[0]}</span>
      </div>
      ...
      {tag && (
        <div style={{ display: "flex", gap: 6, alignSelf: "flex-start", flexWrap: "wrap" }}>
          ...
          {j && <span style={{
            ...
            letterSpacing: ".4px", textTransform: "uppercase",
            background: urgent ? "var(--gj-red)" : "var(--gj-bg)",
            color: urgent ? "#fff" : "var(--gj-grey)",
            border: urgent ? 0 : "1px solid var(--gj-line)",
          }}>{j}</span>}
        </div>
      )}
      ...
      {match && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
          {/* Recommandation qualifiée, sans pourcentage : le score chiffré a été
              retiré des cartes, seule l'explication « Pourquoi ? » subsiste. */}
          <span style={{
            fontSize: 11, fontWeight: 800,
            ...
            <svg className="gj-icon" style={{ width: 11, height: 11 }}><use href="#i-sparkle" /></svg>Recommandé pour toi
          </span>
          <button onClick={() => setWhy(!why)} style={{ background: "transparent", border: 0, padding: 0, fontSize: 11, fontWeight: 800, color: "var(--gj-teal-deep)", cursor: "pointer", fontFamily: "inherit", textDecoration: "underline", textUnderlineOffset: 2 }}>Pourquoi ?</button>
        </div>
      )}
      {match && why && (
        <div style={{ background: "var(--gj-teal-soft)", borderRadius: 8, padding: "8px 10px", fontSize: 11, lineHeight: 1.5, color: "var(--gj-teal-deep)", fontWeight: 600 }}>
          Yaye recommande cette opportunité à partir de ton profil : compétences renseignées, région (Tambacounda) et secteurs suivis — comparés aux 142 opportunités actives.
        </div>
      )}
      {cta && (
        <button style={{
        ...
        {[
          ...
          <div key={i} style={{ ...evRow, borderBottom: i === arr.length - 1 ? 0 : "1px solid var(--gj-line)" }}>
            <div style={dateBox}>
              ...
              <div style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".5px", marginTop: 2 }}>{e.m}</div>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12.5, fontWeight: 800, lineHeight: 1.3 }}>{e.t}</div>
              <div style={{ fontSize: 11, color: "var(--gj-grey)", marginTop: 1 }}>{e.s}</div>
            </div>
          </div>
        ))}
      </div>
        ...
        {[
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: i === arr.length - 1 ? 0 : "1px solid var(--gj-line)" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: "var(--gj-grey)", marginTop: 1 }}>{c.a}</div>
            </div>
    ...
    <div style={panel}>
      ...
      <div style={head}>
        <div style={{
          width: 38, height: 38, borderRadius: "50%",
          background: "linear-gradient(135deg, #19a657, #027f7e)",
          color: "#fff", fontFamily: "var(--gj-font-sans)", fontWeight: 900, fontSize: 19,
          display: "inline-flex", alignItems: "center", justifyContent: "center",
          boxShadow: "0 0 0 2px rgba(255,255,255,.25)",
        }}>Y</div>
        <div style={{ flex: 1, lineHeight: 1.15 }}>
          <div>
            <span style={{ fontFamily: "var(--gj-font-sans)", fontSize: 17, fontWeight: 900, background: "linear-gradient(135deg, #fff, var(--gj-yellow))", WebkitBackgroundClip: "text", color: "transparent" }}>Yaye</span>
            <span style={{ background: "var(--gj-yellow)", color: "var(--gj-ink)", fontSize: 11, fontWeight: 900, padding: "2px 6px", borderRadius: 999, marginLeft: 6, letterSpacing: ".4px" }}>IA</span>
          </div>
          <div style={{ fontSize: 11, opacity: .9, marginTop: 1, display: "inline-flex", alignItems: "center", gap: 5 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#19a657" }} />
      ...
      <div style={body}>
        <div style={{ alignSelf: "center", fontSize: 11, fontWeight: 800, color: "var(--gj-grey)", textTransform: "uppercase", letterSpacing: ".5px", background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 999, padding: "3px 10px", marginBottom: 4 }}>Aujourd'hui · 9:41</div>
...
Object.assign(window, { WebDashboard, WebDashYayePanel, WebDashTopNav, BenefSidebar, BenefTopBar, WebOppCard });
