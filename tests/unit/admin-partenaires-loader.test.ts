...
function raw(over: Partial<OrgRaw> = {}): OrgRaw {
  return {
    ...
    cjsUid: 'rec-1', createdAt: new Date('2026-08-01T00:00:00Z'), ...over,
  }
}
...
describe('GUIC-704 — loader partenaires (helpers purs)', () => {
  ...
  it('mapPartenaireRow : joint agrégats + statut du compte recruteur', () => {
    ...
    expect(r.recruteurStatut).toBe('inactif') // compte suspendu
  ...
  it('kpisPartenaires : tous / vérifiés / non vérifiés / suspendus', () => {
    const rows = [
      mapPartenaireRow(raw({ id: 'a', estVerifie: true }), undefined, 'actif'),
      mapPartenaireRow(raw({ id: 'b', estVerifie: false }), undefined, 'actif'),
      mapPartenaireRow(raw({ id: 'c', estVerifie: true }), undefined, 'inactif'), // suspendu
    ]
    const k = kpisPartenaires(rows)
    expect(k.tous).toBe(3)
    expect(k.verifies).toBe(2)
    expect(k.nonVerifies).toBe(1)
    expect(k.suspendus).toBe(1) // compte recruteur inactif
  })

  it('filtrerPartenaires : suspendus = compte recruteur inactif', () => {
    const rows = [
      mapPartenaireRow(raw({ id: 'a', estVerifie: true }), undefined, 'actif'),
      mapPartenaireRow(raw({ id: 'b', estVerifie: false }), undefined, 'actif'),
      mapPartenaireRow(raw({ id: 'c', estVerifie: true }), undefined, 'inactif'),
    ]
