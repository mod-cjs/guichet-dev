...
beforeAll(async () => {
  await prisma.utilisateur.upsert({ where: { cjsUid: RECRUTEUR_UID }, create: { cjsUid: RECRUTEUR_UID, nom: 'T', prenom: 'A', statut: 'actif' }, update: { statut: 'actif' } })
...
describe('GUIC-689 (É-30) — le chemin recruteur écrit vraiment les compétences', () => {
  ...
  it('créer une offre avec 2 compétences → 2 lignes en base, rattachées à CETTE offre', async () => {
    ...
    const _res = await creerOffreRecruteur({ ...offre, skills: skillIds })
  ...
  it('elles sont marquées REQUISES — c’est ce que la checklist doit pouvoir lire', async () => {
    mockGetSession.mockResolvedValue(session())
    const _res = await creerOffreRecruteur({ ...offre, skills: [skillIds[0]] })
    if (!_res.ok) throw new Error(_res.code)
    const { id } = _res
    creees.push(id)
  ...
  it('sans compétence choisie → aucune ligne parasite', async () => {
    mockGetSession.mockResolvedValue(session())
    const _res = await creerOffreRecruteur({ ...offre })
    if (!_res.ok) throw new Error(_res.code)
    const { id } = _res
    creees.push(id)
