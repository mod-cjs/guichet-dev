...
const CAND_GRAD = { emploi: "linear-gradient(135deg,#1e35ba,#162c5e)", stage: "linear-gradient(135deg,#139ce8,#0B5F8D)", bourse: "linear-gradient(135deg,#ee6f21,#97400F)", format: "linear-gradient(135deg,#19a657,#0E6234)" };
  ...
  return (
    <div onClick={onOpen} style={{ background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 14, padding: 18, cursor: "pointer", display: "flex", flexDirection: "column", gap: 13 }}>
      <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
        <span style={{ width: 52, height: 52, borderRadius: 12, flexShrink: 0, background: CAND_GRAD[c.kind] || CAND_GRAD.emploi, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
          <svg className="gj-icon" style={{ position: "absolute", right: -8, bottom: -9, width: 34, height: 34, opacity: .25 }}><use href={"#" + t.icon} /></svg>
          <svg className="gj-icon" style={{ width: 24, height: 24, position: "relative" }}><use href={"#" + t.icon} /></svg>
        </span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20, marginTop: 20, alignItems: "start" }}>
                ...
                <div key={i} style={{ display: "flex", gap: 16 }}>
                  ...
                  <div style={{ flex: 1, paddingBottom: i < arr.length - 1 ? 18 : 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      ...
                      {ev.current && <span style={{ fontSize: 11, fontWeight: 800, color: ev.st === "refusee" ? "var(--gj-red-ink)" : "var(--gj-teal-deep)", background: ev.st === "refusee" ? "var(--gj-red-soft)" : "var(--gj-teal-soft)", padding: "2px 8px", borderRadius: 999, textTransform: "uppercase", letterSpacing: ".3px" }}>Actuel</span>}
                    </div>
        ...
        <aside style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {c.status !== "acceptee" && c.status !== "refusee" && (
            <button onClick={() => setConfirmRetire(true)} style={{ background: "transparent", color: "var(--gj-red)", border: 0, fontWeight: 800, fontSize: 12.5, cursor: "pointer", fontFamily: "inherit", padding: 6 }}>Retirer ma candidature</button>
          )}
        </aside>
      </div>
      {confirmRetire && (
        <div onClick={() => setConfirmRetire(false)} style={{ position: "absolute", inset: 0, background: "rgba(14,31,27,.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 80 }}>
          <div onClick={(e) => e.stopPropagation()} role="alertdialog" aria-label="Confirmer le retrait" style={{ background: "#fff", borderRadius: 16, padding: 24, width: 400, boxShadow: "0 24px 60px rgba(0,0,0,.3)" }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              <span style={{ width: 42, height: 42, borderRadius: 11, flexShrink: 0, background: "var(--gj-red-soft)", color: "var(--gj-red-ink)", display: "inline-flex", alignItems: "center", justifyContent: "center" }}><svg className="gj-icon" style={{ width: 20, height: 20 }}><use href="#i-alert" /></svg></span>
              <div>
                <h2 style={{ fontSize: 16, fontWeight: 900 }}>Retirer cette candidature ?</h2>
                <p style={{ fontSize: 13, color: "var(--gj-grey)", lineHeight: 1.55, marginTop: 6 }}>Ta candidature à « {c.title} » sera définitivement retirée. Le recruteur en sera informé. Cette action est irréversible.</p>
              </div>
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              <button onClick={() => setConfirmRetire(false)} style={{ background: "#fff", color: "var(--gj-ink)", border: "1.5px solid var(--gj-line)", padding: "11px 18px", borderRadius: 9, fontWeight: 800, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Garder ma candidature</button>
              <button onClick={() => { setConfirmRetire(false); window.gjToast && window.gjToast("Candidature retirée", "error"); }} style={{ background: "var(--gj-red)", color: "#fff", border: 0, padding: "11px 18px", borderRadius: 9, fontWeight: 800, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Retirer</button>
            </div>
          </div>
        </div>
      )}
