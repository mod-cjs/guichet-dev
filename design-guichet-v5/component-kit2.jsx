...
const NavIdentities = () => {
  const roles = [
    ...
    { label: "Admin national", sub: "Barre sombre · accent doré", bg: "#162c5e", bd: "transparent", dot: "var(--gj-yellow)", txt: "#fff" },
  ];
  return (
        ...
        {roles.map((r, i) => (
          <div key={i} style={{ border: "1.5px solid var(--gj-line)", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ background: r.bg, border: r.bd !== "transparent" ? `1px solid ${r.bd}` : 0, padding: 11, display: "flex", flexDirection: "column", gap: 7, minHeight: 116 }}>
              {[0, 1, 2].map((k) => (
                <div key={k} style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 8px", borderRadius: 7, background: k === 0 ? (r.label === "Admin national" ? "linear-gradient(135deg,var(--gj-yellow),#C97F03)" : r.label === "Conseiller CJS" ? "var(--gj-teal)" : r.label === "Recruteur" ? "var(--gj-blue-soft)" : "var(--gj-teal-soft)") : "transparent" }}>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: r.dot }} />
                  <span style={{ flex: 1, height: 6, borderRadius: 3, background: k === 0 && (r.label === "Conseiller CJS" || r.label === "Admin national") ? (r.label === "Admin national" ? "#162c5e" : "#fff") : "currentColor", color: r.txt, opacity: k === 0 ? .9 : .3 }} />
                </div>
              ))}
            </div>
            <div style={{ padding: "9px 11px", borderTop: "1px solid var(--gj-line)" }}><div style={{ fontSize: 12.5, fontWeight: 800, color: swInk }}>{r.label}</div><div style={{ fontSize: 11, color: "var(--gj-grey)", marginTop: 1 }}>{r.sub}</div></div>
          </div>
        ))}
      </div>
    </Block>
...
const MapsAndCard = () => (
  <Block title="Carte CJS & carte du Sénégal" sub="Carte de membre officielle recto/verso (QR) et carte choroplèthe — éléments signature">
    ...
    <Demo label="Carte du Sénégal (présence des centres)" col>
      <div style={{ borderRadius: 14, overflow: "hidden", border: "1.5px solid var(--gj-line)", background: "linear-gradient(135deg,#E2F1F1,#CFE6E6)", aspectRatio: "1.5/1", maxWidth: 420 }}>
        <svg viewBox="0 0 270 210" style={{ width: "100%", height: "100%", padding: 12 }} preserveAspectRatio="xMidYMid meet">
          ...
          <rect x="42" y="125" width="124" height="15" rx="5" fill="#E2F1F1" stroke="var(--gj-teal-deep)" strokeWidth="1" />
        ...
        <div style={{ background: "rgba(0,0,0,.015)", border: "1px solid var(--gj-line)", borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{ padding: 10 }}>
            <div style={{ background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 11, padding: 11, boxShadow: "var(--gj-shadow-sm)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9 }}>{av("AD", 34, "linear-gradient(135deg,var(--gj-teal),var(--gj-teal-deep))")}<div style={{ flex: 1 }}><div style={{ fontSize: 12.5, fontWeight: 800, color: swInk }}>Awa Diop</div><div style={{ fontSize: 11, color: "var(--gj-grey)" }}>Bac+2 · Stats</div></div><span style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-green-ink)", background: "var(--gj-green-soft)", padding: "2px 8px", borderRadius: 999 }}>94%</span></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16, alignItems: "start" }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center", background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 12, padding: 12, marginBottom: 10 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: "linear-gradient(90deg,var(--gj-bg) 25%,#F0F2F5 37%,var(--gj-bg) 63%)", backgroundSize: "400% 100%", animation: "gjsk 1.4s ease infinite" }} />
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 7 }}><div style={{ height: 11, width: "62%", borderRadius: 5, background: "linear-gradient(90deg,var(--gj-bg) 25%,#F0F2F5 37%,var(--gj-bg) 63%)", backgroundSize: "400% 100%", animation: "gjsk 1.4s ease infinite" }} /><div style={{ height: 9, width: "40%", borderRadius: 5, background: "var(--gj-bg)" }} /></div>
        </div>
  ...
  <Block title="Scan QR · calendrier · assistant Yaye · vignette ressource" sub="Composants spécialisés des espaces conseiller, événements, médiathèque">
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 14 }}>
      ...
      <div>
        ...
        <div style={{ position: "relative", aspectRatio: "1/1", borderRadius: 12, background: "#162c5e", overflow: "hidden" }}>
      ...
      <div>
        ...
        <div style={{ border: "1.5px solid var(--gj-line)", borderRadius: 12, padding: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3 }}>
            {["L","M","M","J","V","S","D"].map((d, i) => <div key={i} style={{ textAlign: "center", fontSize: 11, fontWeight: 800, color: "var(--gj-grey)" }}>{d}</div>)}
            {Array.from({ length: 21 }).map((_, i) => { const d = i + 1, has = [5, 10, 14].includes(d); return <div key={i} style={{ aspectRatio: "1/1", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: has ? 900 : 600, background: has ? "var(--gj-teal-soft)" : "transparent", color: has ? "var(--gj-teal-deep)" : "var(--gj-ink)" }}>{d}</div>; })}
          </div>
        </div>
      </div>
      {/* Yaye */}
      ...
      <div>
        ...
        <div style={{ border: "1.5px solid var(--gj-line)", borderRadius: 12, overflow: "hidden" }}>
          <div style={{ height: 74, background: "linear-gradient(135deg,var(--gj-teal-soft),#fff)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
            ...
            <span style={{ position: "absolute", top: 8, right: 8, fontSize: 11, fontWeight: 800, color: "var(--gj-teal-deep)", background: "#fff", padding: "2px 7px", borderRadius: 999 }}>PDF</span>
          </div>
          <div style={{ padding: 10 }}><div style={{ fontSize: 12, fontWeight: 800, color: swInk, lineHeight: 1.25 }}>Guide du CV</div><div style={{ fontSize: 11, color: "var(--gj-grey)", marginTop: 2 }}>24 p. · audio WO</div></div>
        </div>
      </div>
    </div>
  </Block>
...
const NavbarSearch = () => {
  const link = (icon, label, on, badge) => (
    <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px", borderRadius: 9, fontSize: 13, fontWeight: on ? 800 : 600, color: on ? "var(--gj-teal-deep)" : "var(--gj-grey)", background: on ? "var(--gj-teal-soft)" : "transparent" }}>
      ...
      {badge && <span style={{ background: "var(--gj-teal-deep)", color: "#fff", fontSize: 11, fontWeight: 800, padding: "2px 7px", borderRadius: 10 }}>{badge}</span>}
    </div>
  );
  return (
  ...
  <PhoneFrame>
    ...
    <div style={{ flex: 1, overflowY: "auto", background: "var(--gj-bg)", padding: 14, display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div>
        ...
        {/* composant officiel — cjs-card.jsx ; fluide, aucune marge négative requise */}
        <div><MyCJSCard /></div>
      </div>
    </div>
