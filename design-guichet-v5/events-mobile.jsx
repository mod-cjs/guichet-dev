  ...
  return (
    <div style={{ position: "relative", height: h, background: t.grad, color: "#fff", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 12, flexShrink: 0 }}>
      <div style={{ position: "absolute", inset: 0 }}>
        <image-slot id={"evm-" + (ev.id || (ev.title || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 26))} shape="rect" fit="cover" placeholder="Photo réelle de l'événement"></image-slot>
      </div>
      <div style={{ position: "absolute", inset: 0, background: t.grad, opacity: .55, pointerEvents: "none" }} />
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, rgba(0,0,0,.26), rgba(0,0,0,.42))", pointerEvents: "none" }} />
      <svg className="gj-icon" style={{ position: "absolute", right: -12, bottom: -16, width: 92, height: 92, opacity: .16, pointerEvents: "none" }}><use href={"#" + t.icon} /></svg>
      <div style={{ display: "flex", justifyContent: "space-between", position: "relative" }}>
        <div style={{ background: "rgba(255,255,255,.95)", color: t.ink, borderRadius: 9, padding: "5px 9px", textAlign: "center", lineHeight: 1 }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".4px", marginTop: 2 }}>{ev.m}</div>
        </div>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "rgba(0,0,0,.22)", padding: "4px 9px", borderRadius: 999, fontSize: 11, fontWeight: 800, height: "fit-content" }}>
          <svg className="gj-icon" style={{ width: 11, height: 11 }}><use href={"#" + md.icon} /></svg>{ev.mode}
        </span>
      </div>
      <span style={{ position: "relative", alignSelf: "flex-start", display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(255,255,255,.18)", border: "1px solid rgba(255,255,255,.28)", padding: "3px 9px", borderRadius: 999, fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".3px" }}>{t.label}</span>
    </div>
  );
};
    ...
    <div style={{ padding: 13, display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: 10, borderTop: "1px solid var(--gj-line)" }}>
        ...
        {ev.registered
          ...
          : <span style={{ background: "var(--gj-action)", color: "#fff", padding: "8px 15px", borderRadius: 8, fontWeight: 800, fontSize: 12.5 }}>S'inscrire</span>}
      </div>
    </div>
  </div>
);
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, background: "#fff", borderRadius: "20px 20px 0 0", padding: "10px 18px 22px", boxShadow: "0 -8px 32px rgba(0,0,0,.2)", maxHeight: "92%", overflowY: "auto" }}>
          ...
          <div style={{ marginTop: 16 }}>
            <div style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-teal-deep)", textTransform: "uppercase", letterSpacing: ".5px", display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
    ...
    <div style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 13, padding: 13, display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <div style={{ width: 50, textAlign: "center", flexShrink: 0, background: t.soft, color: t.ink, borderRadius: 10, padding: "7px 4px" }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", marginTop: 2 }}>{ev.m}</div>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ fontSize: 11, fontWeight: 800, color: t.ink, background: t.soft, padding: "2px 7px", borderRadius: 999, textTransform: "uppercase" }}>{t.label}</span>
            {past && ev.attended && <span style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-green-ink)", background: "var(--gj-green-soft)", padding: "2px 7px", borderRadius: 999 }}>PRÉSENT·E</span>}
            {past && !ev.attended && <span style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-grey)", background: "var(--gj-bg)", padding: "2px 7px", borderRadius: 999 }}>ABSENT·E</span>}
          </div>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid var(--gj-line)", padding: "0 14px 12px" }}>
        <div style={{ display: "flex", gap: 6, background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 12, padding: 5 }}>
          {tabs.map((t) => {
            const on = t.id === tab;
            return (
              <span key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "9px 0", borderRadius: 8, fontSize: 13, fontWeight: 800, cursor: "pointer", background: on ? "var(--gj-teal-deep)" : "transparent", color: on ? "#fff" : "var(--gj-grey)" }}>
                {t.label}<span style={{ fontSize: 11, fontWeight: 800, background: on ? "rgba(255,255,255,.22)" : "var(--gj-bg)", color: on ? "#fff" : "var(--gj-grey)", padding: "1px 7px", borderRadius: 999 }}>{t.n}</span>
              </span>
            );
          })}
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 14, background: "var(--gj-bg)" }}>
        <div style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 14, padding: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, marginBottom: 6 }}>
            {dow.map((d, i) => <div key={i} style={{ textAlign: "center", fontSize: 11, fontWeight: 800, color: "var(--gj-grey)" }}>{d}</div>)}
          </div>
    ...
    <PhoneFrame>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 14, background: "var(--gj-bg)" }}>
        <div style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 14, padding: 15, display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <span style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-green-ink)", background: "var(--gj-green-soft)", padding: "2px 8px", borderRadius: 999 }}>PRÉSENT·E</span>
