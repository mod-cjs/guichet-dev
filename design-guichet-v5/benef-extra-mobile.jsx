        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 11, background: "var(--gj-bg)" }}>
          ...
          {t.msgs.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: m.me ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "78%", background: m.me ? "var(--gj-teal-deep)" : "#fff", color: m.me ? "#fff" : "var(--gj-ink)", border: m.me ? 0 : "1px solid var(--gj-line)", borderRadius: m.me ? "14px 14px 4px 14px" : "14px 14px 14px 4px", padding: "10px 13px", fontSize: 13.5, lineHeight: 1.45 }}>{m.t}<div style={{ fontSize: 11, opacity: .6, marginTop: 4, textAlign: "right" }}>{m.time}</div></div>
            </div>
          ))}
        </div>
          ...
          <div key={th.id} onClick={() => setOpen(th.id)} style={{ display: "flex", gap: 11, padding: "13px 15px", borderBottom: "1px solid var(--gj-line)", cursor: "pointer" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                <span style={{ fontSize: 14, fontWeight: 800, color: "var(--gj-ink)", display: "inline-flex", alignItems: "center", gap: 5 }}>{th.who}{th.ai && <span style={{ background: "var(--gj-yellow)", color: "var(--gj-ink)", fontSize: 11, fontWeight: 900, padding: "1px 5px", borderRadius: 999 }}>IA</span>}</span>
        ...
        {B_SAVED.map((s) => {
          ...
          return (
            <div key={s.id} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 13, padding: 13, display: "flex", gap: 12 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <span style={{ fontSize: 11, fontWeight: 800, color: g[1], background: g[0], padding: "2px 7px", borderRadius: 999, textTransform: "uppercase" }}>{s.type}</span>
