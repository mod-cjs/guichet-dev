  ...
  <div style={{ padding: "22px 28px 40px", overflowY: "auto", flex: 1 }}>
    <div style={{ maxWidth: 880, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: "22px 18px" }}>
        {ADM_CONTENT.map((ct) => {
          const band = { "Emploi": "#027f7e", "Entrepreneuriat": "#f8a309", "Démarches": "#19a657" }[ct.cat] || "#1e35ba";
          return (
            <div key={ct.id} style={{ display: "flex", flexDirection: "column", gap: 8, cursor: "pointer" }}>
              <div style={{ position: "relative", aspectRatio: "1/1.38", borderRadius: "4px 10px 10px 4px", background: `linear-gradient(150deg, ${band} 0%, ${band} 55%, rgba(0,0,0,.28) 140%)`, boxShadow: "0 10px 22px rgba(10,40,32,.2), inset -2px 0 5px rgba(255,255,255,.12)", overflow: "hidden", display: "flex", flexDirection: "column", padding: "13px 13px 13px 22px" }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,.85)", textTransform: "uppercase", letterSpacing: ".6px" }}>{ct.cat}</span>
                <span style={{ fontSize: 14.5, fontWeight: 900, color: "#fff", lineHeight: 1.25, marginTop: 7, display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{ct.title}</span>
                <span style={{ marginTop: "auto", display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 11, fontWeight: 900, color: band, background: "rgba(255,255,255,.95)", padding: "2px 7px", borderRadius: 5 }}>PDF</span>
                  <span style={{ marginLeft: "auto", fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: ct.statut === "Publié" ? "var(--gj-green-soft)" : "rgba(255,255,255,.9)", color: ct.statut === "Publié" ? "var(--gj-green-ink)" : "var(--gj-grey)" }}>{ct.statut}</span>
                </span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "var(--gj-grey)", fontWeight: 600 }}>
                <svg className="gj-icon" style={{ width: 12, height: 12, color: "var(--gj-grey-2)" }}><use href="#i-download" /></svg>{ct.dl}
                <button style={{ marginLeft: "auto", width: 28, height: 28, borderRadius: 7, border: "1.5px solid var(--gj-line)", background: "#fff", color: "var(--gj-grey)", cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center" }} aria-label="Modifier"><svg className="gj-icon" style={{ width: 13, height: 13 }}><use href="#i-settings" /></svg></button>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {ADM_ROLES.map((r, i) => (
          <div key={i} style={{ ...adCard, padding: 16, display: "flex", alignItems: "center", gap: 14 }}>
            <span style={{ width: 44, height: 44, borderRadius: 11, flexShrink: 0, background: i === 4 ? "linear-gradient(135deg, var(--gj-yellow), #C97F03)" : "var(--gj-teal-soft)", color: i === 4 ? "#162c5e" : "var(--gj-teal-deep)", display: "inline-flex", alignItems: "center", justifyContent: "center" }}><svg className="gj-icon" style={{ width: 21, height: 21 }}><use href={"#" + (i === 4 ? "i-shield" : "i-users")} /></svg></span>
            <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div key={t.id} style={{ ...adCard, padding: 15, display: "flex", alignItems: "center", gap: 14, opacity: t.active ? 1 : 0.62 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: "var(--gj-bg)", color: "var(--gj-grey)" }}>{t.count} actives</span>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: "var(--gj-teal-soft)", color: "var(--gj-teal-deep)" }}><svg className="gj-icon" style={{ width: 11, height: 11 }}><use href="#i-shield" /></svg>{t.moderation}</span>
                </div>
...
const WebAdmin = ({ view = "home" }) => {
  ...
  else if (v === "library") content = <AdminLibrary />;
