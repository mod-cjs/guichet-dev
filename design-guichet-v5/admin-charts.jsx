...
const LineChart = ({ data, labels, color = "var(--gj-teal-deep)", h = 200 }) => {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", display: "block" }} preserveAspectRatio="none">
      ...
      {labels && labels.map((l, i) => <text key={i} x={px(i)} y={h - 6} textAnchor="middle" style={{ fontSize: 11, fill: "var(--gj-grey)", fontWeight: 700 }}>{l}</text>)}
    </svg>
  );
};
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x} y={h - pad.b - bh - 5} textAnchor="middle" style={{ fontSize: 11, fill: "var(--gj-ink)", fontWeight: 800 }}>{d.v}</text>
            <text x={x} y={h - 6} textAnchor="middle" style={{ fontSize: 11, fill: "var(--gj-grey)", fontWeight: 700 }}>{d.m}</text>
          </g>
        );
      })}
    </svg>
...
const SenegalBubbleMap = ({ points = [], height = 360 }) => {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height, borderRadius: 14, overflow: "hidden", background: "linear-gradient(135deg, #E2F1F1, #CFE6E6)", border: "1.5px solid var(--gj-line)" }}>
