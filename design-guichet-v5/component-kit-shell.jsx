...
// Les 16 catégories sont regroupées : une liste plate de 16 entrées n'a plus de
// hiérarchie lisible, et le rail dépasse la hauteur d'écran. Le champ `group`
// déclenche un intertitre à chaque changement de valeur.
const KIT_CATS = [
  { group: "Fondations", id: "charte", label: "Charte Yaakaar 2030", icon: "i-resources", web: ["CharteYaakaar"], mobile: ["MobCharte"] },
  { group: "Fondations", id: "regles-v3", label: "Règles design V3", icon: "i-check-circle", web: ["DesignV3Rules"], mobile: ["MobRegles"] },
  { group: "Fondations", id: "fondations", label: "Couleurs & typographie", icon: "i-image", web: ["Foundations"], mobile: ["MobFoundations"] },
  { group: "Éléments d'interface", id: "boutons", label: "Boutons & actions", icon: "i-bolt", web: ["Buttons", "ButtonStates"], mobile: ["MobButtons", "MobButtonStates"] },
  { group: "Éléments d'interface", id: "formulaires", label: "Formulaires", icon: "i-document", web: ["FormsKit", "FieldStates", "ControlStates"], mobile: ["MobForms", "MobFieldStates"] },
  { group: "Éléments d'interface", id: "onglets", label: "Onglets & filtres", icon: "i-filter", web: ["Tabs", "Chips"], mobile: ["MobTabsScreen"] },
  { group: "Éléments d'interface", id: "navigation", label: "Navigation", icon: "i-menu", web: ["NavbarSearch", "NavIdentities"], mobile: ["MobNavScreen"] },
  { group: "Éléments d'interface", id: "badges", label: "Pastilles & avatars", icon: "i-profile", web: ["Badges", "Avatars"], mobile: ["MobBadges"] },
  { group: "Contenu", id: "cartes", label: "Cartes & listes", icon: "i-home", web: ["Cards", "DataComponents"], mobile: ["MobCardsScreen"] },
  { group: "Contenu", id: "opportunites", label: "Cartes d'opportunité", icon: "i-search", web: ["OppCardsKit"], mobile: ["MobOppCardsKit"] },
  { group: "Contenu", id: "data", label: "Données & progression", icon: "i-trending", web: ["DataViz", "Progress", "KanbanTimeline"], mobile: ["MobData"] },
  { group: "Contenu", id: "signature", label: "Composants signature", icon: "i-pin", web: ["MapsAndCard", "MiscComponents"], mobile: ["MobSignature"] },
  { group: "Surfaces & états", id: "surfaces", label: "Surfaces & overlays", icon: "i-layers", web: ["Surfaces"], mobile: ["MobSurfaces", "MobOverlays"] },
  { group: "Surfaces & états", id: "etats", label: "États & feedback", icon: "i-info", web: ["ChatModalStates"], mobile: ["MobEtats"] },
  { group: "Assistant IA", id: "assistant", label: "Bouton & accessibilité", icon: "i-sparkle", web: ["AssistantIA"], mobile: ["MobAssistantIA"] },
  { group: "Assistant IA", id: "conversation", label: "Conversation IA", icon: "i-chat", web: ["ConversationIA"], mobile: ["MobConversationIA", "MobConvSheet"] },
];
...
// ===== SHELL =====
const KitApp = () => {
  const [cat, setCat] = React.useState("charte");
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "250px 1fr", height: "100vh", background: "var(--gj-bg)", fontFamily: "var(--gj-font-sans)" }}>
      <aside style={sidebar}>
        {/* En-tête empilé : à 250 px de rail, logo + libellé côte à côte
            débordaient depuis le passage du libellé à 11 px. */}
        <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "4px 6px 13px", borderBottom: "1px solid var(--gj-line)", marginBottom: 8 }}>
          <img src="assets/logo-guichet.png" alt="Guichet Jeunesse" style={{ height: 24, width: "auto", alignSelf: "flex-start", maxWidth: "100%" }} />
          <span style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-teal-deep)", letterSpacing: ".5px", textTransform: "uppercase", lineHeight: 1.3, minWidth: 0 }}>Bibliothèque composants</span>
        </div>
        {KIT_CATS.map((k, i) => {
          const on = k.id === cat;
          const newGroup = i === 0 || KIT_CATS[i - 1].group !== k.group;
          return (
            <React.Fragment key={k.id}>
              {newGroup && (
                <div style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-grey)", textTransform: "uppercase", letterSpacing: ".5px", padding: "12px 10px 5px", marginTop: i === 0 ? 0 : 4 }}>{k.group}</div>
              )}
              <button onClick={() => setCat(k.id)} style={{ display: "flex", alignItems: "center", gap: 11, padding: "9px 10px", borderRadius: 9, fontSize: 13, fontWeight: on ? 800 : 600, color: on ? "var(--gj-teal-deep)" : "var(--gj-grey)", background: on ? "var(--gj-teal-soft)" : "transparent", border: 0, cursor: "pointer", fontFamily: "inherit", textAlign: "left", width: "100%", minHeight: 40 }}>
                <svg className="gj-icon" style={{ width: 18, height: 18, flexShrink: 0 }}><use href={"#" + k.icon} /></svg>
                <span style={{ flex: 1, minWidth: 0 }}>{k.label}</span>
                {on && <svg className="gj-icon" style={{ width: 15, height: 15, flexShrink: 0 }}><use href="#i-chevron-right" /></svg>}
              </button>
            </React.Fragment>
          );
        })}
      ...
      <div style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid var(--gj-line)", padding: "0 28px", minHeight: 66, display: "flex", alignItems: "center", gap: 16, flexShrink: 0 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "var(--gj-grey)", marginTop: 1 }}>{c.group} · {view === "web" ? "interface web" : "interface mobile"}</div>
          </div>
