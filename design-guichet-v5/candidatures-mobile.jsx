...
const MOB_CAND_GRAD = { emploi: "linear-gradient(135deg,#1e35ba,#162c5e)", stage: "linear-gradient(135deg,#139ce8,#0B5F8D)", bourse: "linear-gradient(135deg,#ee6f21,#97400F)", format: "linear-gradient(135deg,#19a657,#0E6234)" };
  ...
  return (
    <div onClick={onOpen} style={{ background: "#fff", border: "1px solid var(--gj-line)", borderRadius: 14, padding: 14, cursor: "pointer", display: "flex", flexDirection: "column", gap: 11 }}>
      <div style={{ display: "flex", gap: 11, alignItems: "flex-start" }}>
        <span style={{ width: 46, height: 46, borderRadius: 12, flexShrink: 0, background: MOB_CAND_GRAD[c.kind] || MOB_CAND_GRAD.emploi, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
          <svg className="gj-icon" style={{ position: "absolute", right: -7, bottom: -8, width: 30, height: 30, opacity: .25 }}><use href={"#" + t.icon} /></svg>
          <svg className="gj-icon" style={{ width: 21, height: 21, position: "relative" }}><use href={"#" + t.icon} /></svg>
        </span>
          ...
          {CAND_STATS.map((s, i) => {
            ...
            return (
              <div key={i} style={{ textAlign: "center" }}>
                ...
                <div style={{ fontSize: 11, color: "var(--gj-grey)", marginTop: 3, fontWeight: 700, lineHeight: 1.2 }}>{s.label}</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid var(--gj-line)", padding: "0 14px 12px" }}>
        <div style={{ display: "flex", gap: 6, background: "#fff", border: "1.5px solid var(--gj-line)", borderRadius: 12, padding: 5 }}>
          {tabs.map(([id, label, n]) => {
            const on = id === tab;
            return (
              <span key={id} onClick={() => setTab(id)} style={{ flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "8px 0", borderRadius: 8, fontSize: 11.5, fontWeight: 800, cursor: "pointer", background: on ? "var(--gj-teal-deep)" : "transparent", color: on ? "#fff" : "var(--gj-grey)" }}>
                {label}<span style={{ fontSize: 11, fontWeight: 800, background: on ? "rgba(255,255,255,.22)" : "var(--gj-bg)", color: on ? "#fff" : "var(--gj-grey)", padding: "1px 6px", borderRadius: 999 }}>{n}</span>
              </span>
            );
          })}
        </div>
    ...
    <PhoneFrame>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: "var(--gj-bg)" }}>
        ...
        <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: c.status === "acceptee" ? "var(--gj-green-soft)" : "linear-gradient(135deg, var(--gj-teal-soft), #fff)", border: `1.5px solid ${c.status === "acceptee" ? "var(--gj-green)" : "var(--gj-teal)"}`, borderRadius: 13, padding: 14, display: "flex", gap: 11, alignItems: "flex-start" }}>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 800, color: "var(--gj-grey)", textTransform: "uppercase", letterSpacing: ".4px" }}>Prochaine étape</div>
                ...
                <div key={i} style={{ display: "flex", gap: 13 }}>
                  ...
                  <div style={{ flex: 1, paddingBottom: i < arr.length - 1 ? 16 : 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap" }}>
                      ...
                      {ev.current && <span style={{ fontSize: 11, fontWeight: 800, color: ev.st === "refusee" ? "var(--gj-red-ink)" : "var(--gj-teal-deep)", background: ev.st === "refusee" ? "var(--gj-red-soft)" : "var(--gj-teal-soft)", padding: "2px 7px", borderRadius: 999, textTransform: "uppercase" }}>Actuel</span>}
                    </div>
      ...
      <div style={{ padding: 14, background: "#fff", borderTop: "1px solid var(--gj-line)", display: "flex", gap: 9, flexShrink: 0 }}>
        ...
        {c.status === "entretien" ? (
          ...
          <button onClick={() => setConfirmRetire(true)} style={{ flex: 1, background: "#fff", color: "var(--gj-red)", border: "1.5px solid var(--gj-line)", minHeight: 50, borderRadius: 10, fontWeight: 800, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>Retirer ma candidature</button>
        )}
      </div>
      {confirmRetire && (
        <div onClick={() => setConfirmRetire(false)} style={{ position: "absolute", inset: 0, background: "rgba(14,31,27,.5)", display: "flex", alignItems: "flex-end", zIndex: 90 }}>
          <div onClick={(e) => e.stopPropagation()} role="alertdialog" aria-label="Confirmer le retrait" style={{ background: "#fff", borderRadius: "18px 18px 0 0", padding: "20px 18px 26px", width: "100%" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 9, marginTop: 18 }}>
              <button onClick={() => { setConfirmRetire(false); window.gjToast && window.gjToast("Candidature retirée", "error"); }} style={{ background: "var(--gj-red)", color: "#fff", border: 0, minHeight: 50, borderRadius: 10, fontWeight: 800, fontSize: 14.5, cursor: "pointer", fontFamily: "inherit" }}>Retirer définitivement</button>
              <button onClick={() => setConfirmRetire(false)} style={{ background: "#fff", color: "var(--gj-ink)", border: "1.5px solid var(--gj-line)", minHeight: 50, borderRadius: 10, fontWeight: 800, fontSize: 14.5, cursor: "pointer", fontFamily: "inherit" }}>Garder ma candidature</button>
            </div>
          </div>
        </div>
      )}
